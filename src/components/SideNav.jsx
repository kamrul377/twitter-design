    ...
    return (
        <div className='w-[60px]  md:w-[100px] :bg-white h-screen shadow-sm fixed justify-between flex flex-col'>
            <div className="logos flex flex-col gap-2 my-1">
                {
                    iconLIst.map((ic, i) => {
                        return <div key={i} className='h-12 w-12 rounded-full p-3 bg-transparent hover:bg-slate-200 transition-colors cursor-pointer'>
                            <span key={i}>{ic.icon}</span>
                        </div>
                    })
                }
                <div className='bg-blue-500 h-12 w-12 flex justify-center items-center rounded-full cursor-pointer mx-2'>
                    <TbBrandTwitter color='white' />
                </div>
            </div>

            <div className="profile mx-2 mb-5 ">
                <img title='profile' className='h-12 w-12 rounded-full object-cover hover:animate-pulse hover:cursor-pointer hover:ring-8 ring-gray-200' src="images/kamrul.jpg" alt="" />
            </div>
        </div>
    )
}
